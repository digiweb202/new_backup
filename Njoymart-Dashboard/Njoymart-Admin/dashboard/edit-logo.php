...
<?php
$targetDir = "D:/xampp/htdocs/hit/Mart-Website Frontend/Njoymart-Website-Frontend/nest/demo/assets/imgs/theme/";
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST["submit"])) {
        if (isset($_POST['id'])) {
            ...
            $stmt = $conn->prepare("UPDATE logo SET  link=?" . ($fileName ? ", image_source=?" : "") . " WHERE id=?");
            if ($fileName) {
                $stmt->bind_param("ssi", $link, $fileName, $id);
            } else {
...
if ($id) {
    ...
    if ($result->num_rows > 0) {
        ...
        $existingImage = $row['image_source'];
    } else {
