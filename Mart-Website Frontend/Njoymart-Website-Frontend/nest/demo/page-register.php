...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Perform form validation and other necessary operations
    if (!empty($username) && !empty($email) && !empty($password) && !empty($confirmPassword) && !empty($customerNumber)) {
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            if (strlen($password) >= 6 && $password === $confirmPassword) {
                if (strlen($customerNumber) === 12 && is_numeric($customerNumber)) {
                    $check_query_username = "SELECT * FROM nw_customer WHERE username = '$username'";
                    $result_username = $conn->query($check_query_username);

                    $check_query_email = "SELECT * FROM nw_customer WHERE email = '$email'";
                    $result_email = $conn->query($check_query_email);

                    if ($result_username->num_rows > 0 && $result_email->num_rows > 0) {
                        echo "<script>alert('Error: Username and email already exist. Please use different credentials.');</script>";
                        ...
                        $sql = "INSERT INTO nw_customer (username, email, password, customer_number) VALUES ('$username', '$email', '$password', '$customerNumber')";
                    ...
                    echo "Error: Customer number must be 12 digits.";
                }
            } else {
                echo "Error: Password should be at least 6 characters long and should match the confirm password.";
            }
        } else {
                                ...
                                <div class="login_wrap widget-taber-content background-white">
                                    <div class="padding_eight_all bg-white">
                                        <div class="heading_s1">
                                            <h1 class="mb-5">Signup (Customer)</h1>
                                        ...
                                        <form method="post">
                                            <div class="form-group">
                                                <input type="text" required="" name="username" placeholder="Username" minlength="3" maxlength="50" />
                                            </div>
                                            <div class="form-group">
                                                <input type="email" required="" name="email" placeholder="Email" />
                                            </div>
                                            <div class="form-group">
                                                <input type="text" required="" name="customer_number" placeholder="Customer Number" pattern="\d{12}" title="Please enter a 12-digit number." />
                                            </div>
                                            <div class="form-group">
                                                <input required="" type="password" name="password" placeholder="Your password *" minlength="6" />
                                            </div>
                                            <div class="form-group">
