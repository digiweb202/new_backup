...
if (isset($_SESSION['vendor_id'])) {
    header('Location:  /hit/Njoymart-Dashboard/Njoymart-Admin/dashboard/vendor_panel/index.php');
    exit;
}
// Check connection
if ($conn->connect_error) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $vendor_pan_num = $_POST['vendor_pan_num'] ?? null;
    $vendor_email = $_POST['vendor_email'] ?? null;
    $vendor_mobile = $_POST['vendor_mobile'] ?? null;
    $vendor_password = $_POST['vendor_password'] ?? null;
    $vendor_address = $_POST['vendor_address'] ?? null;
    $vendor_city = $_POST['vendor_city'] ?? null;
    $vendor_state = $_POST['vendor_state'] ?? null;
    $vendor_status = $_POST['vendor_status'] ?? "pending";
    $vendor_Logo = $_POST['vendor_Logo'] ?? "logo.svg";
    $vendor_store_image = $_POST['vendor_store_image'] ?? "banner-2.jpg";
    $vendor_gst_num = $_POST['vendor_gst_num'] ?? null;

    $targetDirFront = "D:/xampp/htdocs/hit/Mart-Website Frontend/Njoymart-Website-Frontend/nest/demo/assets/imgs/pan_front_card/"; // specify the directory for front images
    $targetDirBack = "D:/xampp/htdocs/hit/Mart-Website Frontend/Njoymart-Website-Frontend/nest/demo/assets/imgs/pan_back_card/"; // specify the directory for back images

    $panFrontFile = $targetDirFront . basename($_FILES["pan_front"]["name"]);
    $panBackFile = $targetDirBack . basename($_FILES["pan_back"]["name"]);

    $uploadFrontOk = 1;
    $uploadBackOk = 1;

    $imageFileTypeFront = strtolower(pathinfo($panFrontFile, PATHINFO_EXTENSION));
    $imageFileTypeBack = strtolower(pathinfo($panBackFile, PATHINFO_EXTENSION));

    // Check if file already exists
    if (file_exists($panFrontFile)) {
        echo "Sorry, front file already exists.";
        $uploadFrontOk = 0;
    }
    if (file_exists($panBackFile)) {
        echo "Sorry, back file already exists.";
        $uploadBackOk = 0;
    }

    // Check file size
    if ($_FILES["pan_front"]["size"] > 500000) {
        echo "Sorry, your front file is too large.";
        $uploadFrontOk = 0;
    }
    if ($_FILES["pan_back"]["size"] > 500000) {
        echo "Sorry, your back file is too large.";
        $uploadBackOk = 0;
    }

    // Allow only certain file formats
    $allowedTypes = array('jpg', 'jpeg', 'png', 'gif');
    if (!in_array($imageFileTypeFront, $allowedTypes)) {
        echo "Sorry, only JPG, JPEG, PNG & GIF files are allowed for front image.";
        $uploadFrontOk = 0;
    }
    if (!in_array($imageFileTypeBack, $allowedTypes)) {
        echo "Sorry, only JPG, JPEG, PNG & GIF files are allowed for back image.";
        $uploadBackOk = 0;
    }

    // Check if upload is set to 0 by an error
    if ($uploadFrontOk == 0 || $uploadBackOk == 0) {
        echo "Sorry, your files were not uploaded.";
    } else {
        if (move_uploaded_file($_FILES["pan_front"]["tmp_name"], $panFrontFile) && move_uploaded_file($_FILES["pan_back"]["tmp_name"], $panBackFile)) {
            $fileNameFront = $_FILES["pan_front"]["name"];
            $fileNameBack = $_FILES["pan_back"]["name"];

            $sql = "INSERT INTO vendor (vendor_name, brand_name, vendor_pan_num, vendor_email, vendor_mobile, vendor_password, vendor_address, vendor_city, vendor_state, vendor_status, vendor_Logo, vendor_store_image, vendor_pan_front_img,vendor_pan_back_img, vendor_gst_num) VALUES ('$vendor_name', '$brand_name', '$vendor_pan_num', '$vendor_email', '$vendor_mobile', '$vendor_password', '$vendor_address', '$vendor_city', '$vendor_state', '$vendor_status', '$vendor_Logo', '$vendor_store_image', '$fileNameFront','$fileNameBack', '$vendor_gst_num')";

            if ($conn->query($sql) === TRUE) {
                $last_inserted_id = $conn->insert_id;

                $_SESSION['vendor_id'] = $last_inserted_id;
                $_SESSION['vendor_name'] = $vendor_name;
                $_SESSION['email'] = $vendor_email;
                                ...
                                <div class="login_wrap widget-taber-content background-white">
                                    <div class="padding_eight_all bg-white">
                                        <div class="heading_s1">
                                            <h1 class="mb-5">Signup (Vendor)</h1>
                                            <!-- <p class="mb-30">Don't have an account? <a href="page-register.php">Create here</a></p> -->
                                        </div>
                                        <form method="post" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <input type="text" required name="vendor_name" placeholder="Vendor Username" minlength="3" maxlength="50">
                                            </div>
                                            <div class="form-group">
                                                <input type="email" required name="vendor_email" placeholder="Vendor Email">
                                            </div>
                                            <div class="form-group">
                                                <input type="tel" required name="vendor_mobile" placeholder="Vendor Number" pattern="[0-9]{10}" title="Please enter a 10-digit number.">
                                            </div>
                                            <div class="form-group">
                                                <input type="text" required name="brand_name" placeholder="Vendor Brand Name">
                                            </div>

                                            <div class="form-group">
                                                <input type="text" required name="vendor_address" placeholder="Vendor Address">
                                            </div>

                                            <div class="form-group">
                                                <input type="text" required name="vendor_state" placeholder="Vendor State">
                                            </div>

                                            <div class="form-group">
                                                <input type="text" required name="vendor_city" placeholder="Vendor City">
                                            </div>

                                            <div class="form-group">
                                                <input type="text" required name="vendor_pan_num" placeholder="Vendor PAN Number">
                                            </div>
                                            <div class="form-group">
                                                <input type="text" required name="vendor_gst_num" placeholder="Vendor GST Number">
                                            </div>
                                            <div class="form-group">
                                                <input type="file" name="pan_front" accept="image/*" required>
                                                <label for="pan_front">Upload PAN Front Image</label>
                                            </div>
                                            <div class="form-group">
                                                <input type="file" name="pan_back" accept="image/*" required>
                                                <label for="pan_back">Upload PAN Back Image</label>
                                            </div>

                                            <div class="form-group">
                                                <input required type="password" name="vendor_password" placeholder="Your password *" minlength="6">
                                            </div>
                                            <div class="form-group">
                                                <input required type="password" name="confirm_password" placeholder="Confirm Password" minlength="6">
                                            </div>
                                            <div class="login_footer form-group mb-50">
                                                ...
                                                <a class="text-muted" href="page-vendor-login.php">I already have an account</a>
                                            </div>
                                            <div class="form-group">
